...
echo "Executing: " diffblast.sh $DATABASE "TODO" $QUERY $PREVIOUS_FILE $SEQUENCE_OUTPUT $MAFFT_OUTPUT $RTF_OUTPUT $E_CUTOFF
diffblast.sh $DATABASE "(txid6657 [ORGN]) NOT (txid6830 [ORGN])" $QUERY $PREVIOUS_FILE $SEQUENCE_OUTPUT $MAFFT_OUTPUT $RTF_OUTPUT $ECUTOFF
